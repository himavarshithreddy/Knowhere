...
async function main() {
  const url = "https://www.instagram.com/reels/DZyzCq0vwS8/embed/";
  ...
  // Look for image inside embed
  console.log("Image:", $("img.EmbeddedMediaImage").attr("src"));
  ...
  for (const s of scripts) {
    ...
    if (content && content.includes("window.__additionalDataLoaded")) {
      ...
      if (match) {
        ...
        if (media) {
          console.log("Extracted title:", media.title);
